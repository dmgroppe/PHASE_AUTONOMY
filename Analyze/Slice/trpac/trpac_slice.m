function [R] = trpac_slice(data_ap, dosave)
...
for i=1:ncond
    ...
    h = figure(i);clf;
    [R(i).pac, R(i).pc, R(i).pk, R(i).corr, R(i).clfrange] = sl_trpac_v3(S.data, data_ap.cond.times(:,i), S.srate,  data_ap.chlabels(1:2), false);
    R(i).cond = data_ap.cond.names{i};
    fname = sprintf('%s - TRPAC %s', upper(data_ap.cond.fname{i}), data_ap.cond.names{i});
    set(h, 'Name', fname);
    if dosave
