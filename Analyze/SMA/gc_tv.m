function [ret] = gc_tv(EEG, channels, ptname, cond, freqs)
...
Fs      =   EEG.srate;        % sampling frequency  (for spectral analysis only)
%freqs   =   50:250;    % frequency range to analyze (spectral analysis only)
...
figure(4); clf reset;
nGW = GW/(max(max(max(GW))));
cca_plotcausality_spectral(nGW,freqs);
