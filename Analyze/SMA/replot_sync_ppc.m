...
beta = nlinfit(R.x,crho(findex,:),@cos_fit,[0.5 0]);
pfit = cos_fit(beta,R.x(1):.1:R.x(end));
hold on;
...
mean_phase = circ_mean(R.mean_angle);
...
for i=1:nranges
    ...
    mean_phase = circ_mean(R.mean_angle(ind));
    fprintf('\n  Mean phase %6.2f', mean_phase);
    fprintf('\n  Delay = %6.2f +- %6.2e ms', meandelay, semdelay);
end
fprintf('\n');

%% PLOT the zero phase CORR value - using the fit
h = figure(5);
clf('reset');
f3name = [fname, ' ZERO - FITTED'];
set(h, 'Name', f3name);
label.x = 'Frequency';
label.y = 'Power Corr';
label.title = 'FITTED - Power Corr at GOOD phase';

[amps] = sync_ppc_amps(R.x, R.rho);

zindex = find(R.x == 0);
plot_xy(R.ap.freqs, amps, label, ap.yaxis);
set(gca, 'TickDir', 'out');
