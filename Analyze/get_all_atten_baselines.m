...
switch which_cond
    case ''
        ...
        otherwise
            display(sprintf('Collecting baselines for %s test.', upper(which_test)));
            baselines = get_spectra(aparams, which_test, 'scene');
            baselines = add_baselines(baselines, aparams, which_test, 'face');
        end
    otherwise
        display(sprintf('Collecting baselines for %s-%s test.', upper(which_test), upper(which_cond)));
        baselines = get_spectra(aparams, which_test, which_cond);    
end
