function [] = sl_mi_grid(data, sr, lfrange, hfrange, seglength, dop)
...
%             if dop
%                 [MI(k,j,i), p(k,j,i), surr_mi(j,i)] = sync_mi(lfphase(segs(k,1):segs(k,2)), hf_env(j,segs(k,1):segs(k,2)), nbins, nsurr, 1);
%             else
%                 [MI(k,j,i), p(k,j,i), ~] = sync_mi(lfphase(segs(k,1):segs(k,2)), hf_env(j,(segs(k,1):segs(k,2))), nbins, nsurr, 0);
%             end
%         end
%     end
% end
