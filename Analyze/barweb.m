...
if size(barvalues,1) ~= size(errors,1) || size(barvalues,2) ~= size(errors,2)
    ...
    if max(max(barvalues+errors)) > 0
        ylim([0 ymax*1.1]);
    else
        ylim([min(min(barvalues+errors))*1.1 0]);
    end
	...
	if ~isempty(bw_title)
		title(bw_title, 'fontsize',8, 'fontweight','b');
	end
	if ~isempty(bw_xlabel)
		xlabel(bw_xlabel, 'fontsize',8);
	end
	if ~isempty(bw_ylabel)
		ylabel(bw_ylabel, 'fontsize',8);
	end
